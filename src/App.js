...
function App() {
  ...
  const handleComplete = (e) => {
    ...
    let index = newTodo.findIndex(par => par.id === e)
  ...
  return (
    <div className='flex flex-col'>
      <h1 className='inline m-5 text-4xl md:mt-10 mb-10 text-center'>Routines</h1>
      <form>
      ...
      <div className="flex m-0 justify-center">
        <div className="Container md:w-96 md:h-8/12 flex w-11/12 flex-col  h-72 justify-start overflow-y-auto justify-items-strecth">
          {
            todoData.map(
            (todo) => 
            <div className='flex justify-between'>
            
            <p key={todo.id} className={`mx-5 my-2 text-xl duration-1000`}>{todo.task}</p> 
            <Icon onClick={()=>handleComplete(todo.id)} icon="material-symbols:check-box" width="35" height="35"  className='mr-2 text-gray-300 active:text-gray-700 hover:text-gray-500' />
